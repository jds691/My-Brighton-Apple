//
//  LargeTimetableWidgetView.swift
//  My Brighton
//
...
struct LargeTimetableWidgetView: View {
    ...
    var body: some View {
        ViewThatFits(in: .vertical) {
            VStack(alignment: .leading) {
                widgetTitle
                    .font(.headline)
                verticalList(Array(entry.classes.prefix(rowLimit)))

                let additionalClasses = Array(entry.classes.dropFirst(rowLimit))
    ...
    private var rowLimit: Int {
        dynamicTypeSize.isAccessibilitySize ? 4 : 5
    }
}
